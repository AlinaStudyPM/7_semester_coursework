# benchmarks/eval_rerank_dragon.py
"""
RQ3: Влияние реранкера (top_rerank) при фиксированном top_embed=20.
...
Сравниваем стратегии:
  — top_rerank=1,3,5,10 (реранкер включён)
  — top_rerank=20 (эквивалент «без реранкера»)

График: 2×2 bar-charts (Precision, Recall, Latency, Memory).
"""
...
def main():
    parser = argparse.ArgumentParser(description="RQ3: Влияние top_rerank при фиксированном top_embed")
    ...
    parser.add_argument("--top-embed", type=int, default=cfg.TOP_EMBED_FIXED,
                        help="Фиксированное значение top_embed")
    parser.add_argument("--top-rerank-values", nargs="+", type=int, default=cfg.TOP_RERANK_VALUES,
                        help="Список значений top_rerank (например: 1 3 5 10 20)")
    ...
    parser.add_argument("--out-dir", default="./benchmarks/results/rerank", help="Папка для результатов")
    ...
    # ------------------------------------------------------------------
    print("=" * 60)
    print("RQ3: Влияние реранкера при фиксированном top_embed")
    print("=" * 60)
    ...
    logger.log_config({
        "experiment": "RQ3_rerank",
        "model_gen": args.model,
        "embedding_model": args.embedding,
        "prompt_file": args.prompt,
        "top_embed_fixed": args.top_embed,
        "top_rerank_values": args.top_rerank_values,
    ...
    # ------------------------------------------------------------------
    # 6. Основной цикл: один проход на каждое значение top_rerank
    # ------------------------------------------------------------------
    ...
    for top_rerank in args.top_rerank_values:
        ...
        print(f"\n>>> top_embed={args.top_embed}, {mode_str}")
        ...
        # Batch-метрики RAGAS
        print(f"     Вычисление метрик для {mode_str}...")
        ...
        for idx in range(len(qs)):
            samples.append({
                ...
                "scores": {
                    "context_precision": round(cp_scores[idx], 3) if cp_scores[idx] is not None else None,
                    "context_recall": round(cr_scores[idx], 3) if cr_scores[idx] is not None else None,
                },
            })

        samples_by_rerank[top_rerank] = samples

        for s in samples:
            logger.log_sample({"top_rerank": top_rerank, "use_rerank": use_rerank, **s})
    ...
    all_samples = []
    for top_rerank, samples in samples_by_rerank.items():
        for s in samples:
            all_samples.append({"top_rerank": top_rerank, **s})
    ...
    # 8. Агрегация
    # ------------------------------------------------------------------
    agg = ResultRerank(samples_by_rerank)
    summary = agg.compute_summary()

    md_path = logger.run_dir / f"{logger.run_id}_report.md"
    agg.to_markdown(md_path, top_embed_fixed=args.top_embed)
    ...
    for i, tr in enumerate(summary["top_reranks"]):
        mode = "без rerank" if tr == args.top_embed else "rerank"
        print(f"\ntop_rerank = {tr} ({mode})")
        cp = summary["context_precision"][i]
        cr = summary["context_recall"][i]
        lat = summary["latency"][i]
        mem = summary["memory"][i]
